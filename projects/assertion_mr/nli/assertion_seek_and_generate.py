...
from jack.core import TensorPort, Ports, SharedResources, TensorPortTensors
...
from projects.assertion_mr.nli.shared import MultipleChoiceAssertionInputModule, nli_model
...
class NLIAssertionSeekingAndGeneratingModel(TFModelModule):
    ...
    def create_output(self, shared_resources, input_tensors):
        with tf.variable_scope('assertion_reader') as vs:
            ...
            question_arg_span = tensors.question_arg_span
            support_arg_span = tensors.support_arg_span
            assertion2question_arg_span = tensors.assertion2question_arg_span
            assertion2support_arg_span = tensors.assertion2support_arg_span
            word2lemma = tensors.word2lemma
            predicate2words = tensors.predicate2words
            ...
            q_outputs = sequence_encoder.bi_lstm(size, emb_question, question_length, name="q_rnn_selection")
            s_outputs = sequence_encoder.bi_lstm(size, emb_support, support_length, name="s_rnn_selection")
            ...
            for i in range(max_pairs):
                ...
                def generate_assertions():
                    ...
                    q_arg_length, q_arg_start = tf.div(pair_idx, max_question_length), tf.mod(pair_idx,
                                                                                              max_question_length)
                    ...
                    if shared_resources.config.get('generator') is not None:
                        gen_assertions, gen_assertion_length, gen_log_p = _generate_assertion(
                            ...
                            word_embeddings, word_chars, word_char_length, predicate2words, is_eval)
                        ...
                        gen_assertions, gen_assertion_length = _construct_assertions(
                            args1, args2, q_arg_length + 1, s_arg_length + 1, gen_pred, gen_pred_length,
                            predicate2words)
                ...
                with tf.device('/cpu:0'):
                    ...
                    assertion_lengths = tf.concat([assertion_lengths, gen_assertion_length], axis=0)
                    max_assertion_length = tf.reduce_max(assertion_lengths)

                    def pad(s):
                        s_length = tf.shape(s)[1]
                        return tf.cond(s_length < max_assertion_length,
                                       lambda: tf.concat(
                                           [s, tf.zeros([tf.shape(s)[0], max_assertion_length - s_length],
                                                        dtype=tf.int32)],
                                           1),
                                       lambda: s)
                    ...
                    new_selected_assertions = tf.concat([
                        ...
                        tf.fill([tf.shape(assertion_lengths)[0] - tf.shape(new_selected_assertions)[0]], True)], axis=0)
            ...
            final_logits, all_logits = tf.cond(
                ...
                lambda: self._compute_logits(
                    ...
                    assertions, assertion_lengths, assertion2question_plus, logits),
                lambda: self._compute_logits(
                    shared_resources, word2lemma, new_word_embeddings, question, question_length,
            ...
            tf.summary.scalar("fraction_possible_kb_pairs",
                              ...
                              tf.reduce_sum(
                                  tf.minimum(tf.reduce_sum(tf.to_float(tf.logical_not(assertion_pair_mask)), 1),
                                             max_pairs)) + 1e-6)
        ...
        return TensorPort.to_mapping(self.output_ports, (
            final_logits, tf.argmax(final_logits, 1), all_logits, tf.stack(log_ps),
    ...
    def _compute_logits(self, shared_resources, word2lemma, word_embeddings,
        ...
        assertion_refined_embeddings, _, offsets = embedding_refinement(
            size, gathered_embeddings, reading_encoder_config,
    ...
    def create_training_output(self, shared_resources, input_tensors):
        ...
        max_span = shared_resources.config.get('max_span', 5)
        pair_selection_loss = self.pair_selection_loss(
            tensors.assertion2question, tensors.assertion2question_arg_span,
            tensors.assertion2support_arg_span, tensors.span_pair_scores, max_span, tensors.question,
            tensors.question_arg_span, tensors.support, tensors.support_arg_span)

        # Train actual model
        labels_3D = tf.tile(tf.expand_dims(tensors.target_index, 0), [tf.shape(tensors.logits_3D)[0], 1])
        weights = tf.to_float(tensors.assertion_selection_mask)
        losses = tf.losses.sparse_softmax_cross_entropy(logits=tensors.logits_3D, labels=labels_3D,
                                                        reduction=tf.losses.Reduction.NONE)
        loss = tf.losses.sparse_softmax_cross_entropy(logits=tensors.logits, labels=tensors.target_index)
        ...
        policy_loss = tf.reduce_mean(tf.reduce_sum(
            tensors.policy_log_probs * tf.stop_gradient(neg_gain - baseline) * weights[:-1], axis=0))
        ...
        tf.summary.scalar("policy_prob",
                          tf.reduce_sum(tf.exp(tensors.policy_log_probs) * weights[:-1]) /
        ...
        if shared_resources.config.get('only_policy'):
            return {Ports.loss: policy_loss + baseline_loss + pair_selection_loss}
        else:
            return {Ports.loss: loss + policy_loss + baseline_loss + pair_selection_loss}
...
def _generate_assertion(size, with_char_embeddings, max_conv_width, num_chars,
    ...
    logits, predicate_idx, predicted_predicates, pred_lengths, rev_ordering_logits = predicate_classification_generator(
        size, num_predicate_tokens, with_char_embeddings, full_predicate_vocab, predicate_vocab,
